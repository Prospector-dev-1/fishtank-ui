...
export default function Home() {
  ...
  const loadUserData = async () => {
    ...
    if (user) {
      ...
      // Load user stats
      const { data: innovations } = await supabase.from("innovations").select("id").eq("user_id", user.id);
      const { data: pitches } = await supabase.from("pitches").select("id").eq("user_id", user.id);
      setStats({
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="section-padding">
        ...
        <div className="grid grid-cols-1 gap-3">
          {quickActions.map((action, idx) => (
            <Card
              key={idx}
              className="ios-card p-4 cursor-pointer hover-lift active:scale-95 transition-smooth"
          ...
          <div className="space-y-4">
            {recentActivity.map((activity, idx) => (
              <div key={idx} className="flex items-start gap-3">
                <div
                  className={`w-8 h-8 rounded-full bg-muted flex items-center justify-center flex-shrink-0 ${activity.color}`}
                >
        ...
        <div className="swipe-container gap-3 -mx-4 px-4">
          {[
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1">{Math.floor(Math.random() * 50) + 10} projects</p>
            </Card>
          ))}
        </div>
      </div>
      ...
      <div className="section-padding">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card
            className="ios-card p-6 cursor-pointer hover-lift text-center"
          ...
          <Card
            className="ios-card p-6 cursor-pointer hover-lift text-center"
            onClick={() => navigate("/innovator/collaborate")}
          >
