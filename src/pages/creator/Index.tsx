...
import { Briefcase, Users, MessageCircle, TrendingUp, Star, Clock, DollarSign, Plus, Calendar, CheckCircle, CheckCircle2, Palette, Code, Megaphone, Brain, Film, Mail, UserPlus, X, Check, Settings } from "lucide-react";
...
const Index = () => {
  ...
  const loadEarnings = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from("earnings").select("*").eq("creator_id", user.id).order("created_at", {
        ascending: false
      });
      if (error) throw error;
      setEarnings(data || []);
  ...
  const loadMilestones = async () => {
    ...
    try {
      ...
      } = await supabase.from("contracts").select("id").eq("creator_id", user.id);
      ...
      if (contracts && contracts.length > 0) {
        ...
        } = await supabase.from("milestones").select("*, contracts(title)").in("contract_id", contractIds).in("status", ["pending", "in_progress", "submitted"]).order("due_date", {
          ascending: true
        });
  ...
  const loadInvitations = async () => {
    ...
    try {
      ...
      const {
        ...
        error: invError
      } = await supabase.from("invitations").select("*").eq("to_creator_id", user.id).eq("status", "pending").order("created_at", {
        ascending: false
      });
  ...
  const loadFriendRequests = async () => {
    ...
    try {
      ...
      const {
        data: requests,
        error: requestError
      } = await supabase.from("friend_requests").select("*").eq("to_user_id", user.id).eq("status", "pending").order("created_at", {
        ascending: false
      });
  ...
  const releasedEarnings = earnings.filter(e => e.status === "released").reduce((sum, e) => sum + Number(e.amount), 0);
  ...
  const thisMonthEarnings = earnings.filter(e => new Date(e.created_at) >= currentMonthStart).reduce((sum, e) => sum + Number(e.amount), 0);
  ...
  const pendingReleases = earnings.filter(e => e.status === "pending").map(e => ({
    id: e.id,
  ...
  return <div className="pb-20 px-4 pt-6 max-w-md mx-auto space-y-6 relative animate-fade-in">
      <ProfileHeader />
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              ...
              <div className="space-y-3">
                <Button className="w-full bg-foreground hover:bg-foreground/90 text-background shadow-sm" onClick={() => navigate("/creator/discover")}>
                  <TrendingUp className="h-5 w-5 mr-2" />
                  Browse Projects
                </Button>
                <Button variant="outline" className="w-full border-border/50" onClick={() => navigate("/creator/settings")}>
      ...
      <div className="grid grid-cols-2 gap-4 animate-slide-up">
        {metrics.map((metric, index) => <Card key={index} className="p-6 text-center shadow-card hover:shadow-elegant transition-all duration-200 cursor-pointer active:scale-[0.98]" onClick={() => {
        ...
        if (metric.label === "Active Jobs") navigate("/discover");
            ...
            <CardContent className="p-0">
              ...
              {metric.progress && <div className="w-full bg-muted/30 rounded-full h-2">
      ...
      <Card className="bg-gradient-primary text-white shadow-elegant animate-scale-in cursor-pointer hover:shadow-2xl transition-all duration-300 hover:scale-[1.02]" onClick={() => navigate('/creator/earnings')}>
        <CardContent className="p-6">
          ...
          <Button variant="secondary" size="sm" className="w-full bg-white/20 hover:bg-white/30 text-white border-white/30" onClick={e => {
          e.stopPropagation();
      ...
      {/* Friend Requests */}
      {friendRequests.length > 0 && <Card className="shadow-card animate-slide-up">
          ...
          <CardContent className="space-y-3">
            {friendRequests.map(request => <div key={request.id} className="p-4 rounded-xl bg-gradient-subtle border border-accent/10 hover:shadow-elegant transition-all duration-200">
                <div className="flex items-center gap-3">
                  <img src={request.avatar} alt={request.name} className="w-12 h-12 rounded-full border-2 border-accent/20 cursor-pointer" onClick={() => navigate(`/creator/creators/${request.from_user_id}`)} />
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-base cursor-pointer hover:text-primary transition-colors" onClick={() => navigate(`/creator/creators/${request.from_user_id}`)}>
                      {request.name}
                    </h4>
      ...
      {/* Invitations */}
      {activeInvitations.length > 0 && <Card className="shadow-card animate-slide-up">
          <CardHeader className="pb-3">
            <CardTitle className="text-lg flex items-center gap-2">
          ...
          <CardContent className="space-y-3">
            {activeInvitations.map(invitation => <div key={invitation.id} className="p-4 rounded-xl bg-gradient-subtle border border-primary/10 hover:shadow-elegant transition-all duration-200">
                <div className="flex items-start gap-3">
                  <img src={invitation.avatar} alt={invitation.from} className="w-12 h-12 rounded-full border-2 border-primary/20" />
                  <div className="flex-1 min-w-0">
      ...
      <div className="space-y-4 animate-slide-up">
        ...
        <div className="space-y-3">
          {projects.length > 0 ? projects.map(project => <Card key={project.id} className="p-4 shadow-card hover:shadow-elegant transition-all duration-200">
                <CardContent className="p-0">
      ...
      {/* Quick Actions */}
      <div className="grid grid-cols-2 gap-4 animate-slide-up">
        <Button variant="outline" className="h-auto p-6 flex flex-col gap-3 shadow-card hover:shadow-elegant transition-all duration-200" onClick={() => navigate("/creator/proposals")}>
          <div className="bg-primary/10 p-3 rounded-full">
            <Plus className="h-6 w-6 text-primary" />
          </div>
          <span className="text-sm font-semibold">Post Service</span>
        </Button>
        <Button variant="outline" className="h-auto p-6 flex flex-col gap-3 shadow-card hover:shadow-elegant transition-all duration-200" onClick={() => navigate("/creator/settings")}>
